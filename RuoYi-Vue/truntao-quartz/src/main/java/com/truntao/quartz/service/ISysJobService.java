package com.truntao.quartz.service;


import com.github.pagehelper.Page;
import com.truntao.quartz.domain.dto.SysJobDTO;
import com.truntao.quartz.domain.ro.SysJobParam;
import com.truntao.quartz.domain.ro.SysJobUpdateParam;
import org.quartz.SchedulerException;
import com.truntao.common.exception.job.TaskException;
import com.truntao.quartz.domain.po.SysJob;

/**
 * 定时任务调度信息信息 服务层
 *
 * @author truntao
 */
public interface ISysJobService {
    /**
     * 获取quartz调度器的计划任务
     *
     * @param jobParam 调度信息
     * @return 调度任务集合
     */
    Page<SysJobDTO> selectJobList(SysJobParam jobParam);

    /**
     * 通过调度任务ID查询调度信息
     *
     * @param jobId 调度任务ID
     * @return 调度任务对象信息
     */
    SysJobDTO selectJobById(Long jobId);

    /**
     * 暂停任务
     *
     * @param job 调度信息
     * @return 结果
     */
    int pauseJob(SysJob job) throws SchedulerException;

    /**
     * 恢复任务
     *
     * @param job 调度信息
     * @return 结果
     */
    int resumeJob(SysJob job) throws SchedulerException;

    /**
     * 删除任务后，所对应的trigger也将被删除
     *
     * @param job 调度信息
     * @return 结果
     */
    int deleteJob(SysJob job) throws SchedulerException;

    /**
     * 批量删除调度信息
     *
     * @param jobIds 需要删除的任务ID
     * @return 结果
     */
    void deleteJobByIds(Long[] jobIds) throws SchedulerException;

    /**
     * 任务调度状态修改
     *
     * @param jobUpdateParam 调度信息
     * @return 结果
     */
    int changeStatus(SysJobUpdateParam jobUpdateParam) throws SchedulerException;

    /**
     * 立即运行任务
     *
     * @param job 调度信息
     * @return 结果
     */
    boolean run(SysJob job) throws SchedulerException;

    /**
     * 新增任务
     *
     * @param jobParam 调度信息
     * @return 结果
     */
    int insertJob(SysJobParam jobParam) throws SchedulerException, TaskException;

    /**
     * 更新任务
     *
     * @param jobUpdateParam 调度信息
     * @return 结果
     */
    int updateJob(SysJobUpdateParam jobUpdateParam) throws SchedulerException, TaskException;

    /**
     * 校验cron表达式是否有效
     *
     * @param cronExpression 表达式
     * @return 结果
     */
    boolean checkCronExpressionIsValid(String cronExpression);
}
