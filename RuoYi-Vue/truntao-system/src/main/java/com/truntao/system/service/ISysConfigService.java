package com.truntao.system.service;

import com.github.pagehelper.Page;
import com.truntao.system.domain.dto.SysConfigDTO;
import com.truntao.system.domain.ro.SysConfigParam;
import com.truntao.system.domain.ro.SysConfigUpdateParam;

/**
 * 参数配置 服务层
 *
 * @author truntao
 */
public interface ISysConfigService {
    /**
     * 查询参数配置信息
     *
     * @param configId 参数配置ID
     * @return 参数配置信息
     */
    SysConfigDTO selectConfigById(Long configId);

    /**
     * 根据键名查询参数配置信息
     *
     * @param configKey 参数键名
     * @return 参数键值
     */
    String selectConfigByKey(String configKey);

    /**
     * 获取验证码开关
     *
     * @return true开启，false关闭
     */
    boolean selectCaptchaEnabled();

    /**
     * 查询参数配置列表
     *
     * @param configParam 参数配置信息
     * @return 参数配置集合
     */
    Page<SysConfigDTO> selectConfigList(SysConfigParam configParam);

    /**
     * 新增参数配置
     *
     * @param config 参数配置信息
     * @return 结果
     */
    int insertConfig(SysConfigParam configParam);

    /**
     * 修改参数配置
     *
     * @param configUpdateParam 参数配置信息
     * @return 结果
     */
    int updateConfig(SysConfigUpdateParam configUpdateParam);

    /**
     * 批量删除参数信息
     *
     * @param configIds 需要删除的参数ID
     */
    void deleteConfigByIds(Long[] configIds);

    /**
     * 加载参数缓存数据
     */
    void loadingConfigCache();

    /**
     * 清空参数缓存数据
     */
    void clearConfigCache();

    /**
     * 重置参数缓存数据
     */
    void resetConfigCache();

    /**
     * 校验参数键名是否唯一
     *
     * @param configId  参数信息Id
     * @param configKey 参数信息Key
     * @return 结果
     */
    boolean checkConfigKeyUnique(Long configId, String configKey);
}
