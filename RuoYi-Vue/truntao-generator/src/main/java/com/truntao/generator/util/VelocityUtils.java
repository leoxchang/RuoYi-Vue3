...
public class VelocityUtils {
     ...
     * vue3 模版
     */
    private static final String VUE3_VM = "/v3/index.vue.vm";
    /**
     * vue 模版
     */
    private static final String VUE_VM = "/index.vue.vm";
    ...
    public static void setSubVelocityContext(VelocityContext context, GenTableDTO genTable) {
        ...
        String subTableFkClassName = CharSequenceUtil.upperFirst(CharSequenceUtil.toCamelCase(subTableFkName));
    ...
    public static List<String> getTemplateList(String tplCategory, String tplWebType) {
        ...
        templates.add("vm/java/roUpdate.java.vm");
        ...
        if (GenConstants.TPL_CRUD.equals(tplCategory)) {
            templates.add(useWebType + VUE3_VM);
            templates.add(useWebType + VUE_VM);
    ...
    public static String getFileName(String template, GenTableDTO genTable) {
        ...
        String javaPath = PROJECT_PATH + FileSystems.getDefault().getSeparator() + StringUtils.replace(packageName, ".", "/");
        String mybatisPath = MYBATIS_PATH + FileSystems.getDefault().getSeparator() + moduleName;
        String vuePath = "vue";

        if (template.contains("domain.java.vm")) {
            fileName = CharSequenceUtil.format("{}/domain/po/{}.java", javaPath, className);
        }
        if (template.contains("dto.java.vm")) {
            fileName = CharSequenceUtil.format("{}/domain/dto/{}DTO.java", javaPath, className);
        }
        if (template.contains("ro.java.vm")) {
            fileName = CharSequenceUtil.format("{}/domain/ro/{}Param.java", javaPath, className);
        }
        if (template.contains("roUpdate.java.vm")) {
            fileName = CharSequenceUtil.format("{}/domain/ro/{}UpdateParam.java", javaPath, className);
        }
        if (template.contains("sub-domain.java.vm") && StringUtils.equals(GenConstants.TPL_SUB,
                genTable.getTplCategory())) {
            fileName = CharSequenceUtil.format("{}/domain/po/{}.java", javaPath, genTable.getSubTable().getClassName());
        ...
        } else if (template.contains(VUE3_VM)) {
            fileName = CharSequenceUtil.format("{}/views/v3/{}/{}/index.vue", vuePath, moduleName, businessName);
        } else if (template.contains("/v3/index-tree.vue.vm")) {
            fileName = CharSequenceUtil.format("{}/views/v3/{}/{}/index.vue", vuePath, moduleName, businessName);
        } else if (template.contains(VUE_VM)) {
     ...
     * @return 返回需要导入的包列表
     */
    public static Set<String> getImportList(GenTableDTO genTable) {
        ...
        for (GenTableColumnDTO column : columns) {
            if (!GenUtils.isSuperColumn(column.getJavaField()) && GenConstants.TYPE_DATE.equals(column.getJavaType())) {
                importList.add("java.util.Date");
                importList.add("com.fasterxml.jackson.annotation.JsonFormat");
            } else if (!GenUtils.isSuperColumn(column.getJavaField()) && GenConstants.TYPE_BIG_DECIMAL.equals(column.getJavaType())) {
     ...
     * @param columns 列集合
     */
    public static void addDicts(Set<String> dicts, List<GenTableColumnDTO> columns) {
        for (GenTableColumnDTO column : columns) {
            if (!GenUtils.isSuperColumn(column.getJavaField()) && StringUtils.isNotEmpty(column.getDictType()) && StringUtils.equalsAny(
                    column.getHtmlType(),
    ...
    public static int getExpandColumn(GenTableDTO genTable) {
        ...
        int num = 0;
        for (GenTableColumnDTO column : genTable.getColumns()) {
            if (GenUtils.isList(column.getList())) {
                num++;
