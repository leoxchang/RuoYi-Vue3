...
public class GenTableServiceImpl implements IGenTableService {
    ...
    public GenTableDTO selectGenTableById(Long id) {
        GenTableDTO genTable = genTableMapper.selectGenTableById(id);
     ...
     */
    @Override
    public Page<GenTableDTO> selectGenTableList(GenTableParam genTableParam) {
        return genTableMapper.selectGenTableList(genTableParam.getGenTable());
    }
     ...
     */
    @Override
    public Page<GenTableDTO> selectDbTableList(GenTableParam genTableParam) {
        return genTableMapper.selectDbTableList(genTableParam.getGenTable());
    }
    ...
    public void updateGenTable(GenTableParam genTableParam) {
        ...
        if (row > 0) {
            for (GenTableColumn genTableColumn : genTableParam.getColumns()) {
            ...
            for (GenTable table : tableList) {
                ...
                if (row > 0) {
                    ...
                    for (GenTableColumn column : genTableColumns) {
                        ...
                        column.setCreateBy(SecurityUtils.getLoginUser().getUsername());
    ...
    public void syncDb(String tableName) {
        GenTableDTO table = genTableMapper.selectGenTableByName(tableName);
        List<GenTableColumnDTO> tableColumns = table.getColumns();
        Map<String, GenTableColumnDTO> tableColumnMap =
                tableColumns.stream().collect(Collectors.toMap(GenTableColumnDTO::getColumnName, Function.identity()));
        ...
        List<String> dbTableColumnNames =
                dbTableColumns.stream().map(GenTableColumn::getColumnName).toList();

        dbTableColumns.forEach(column -> {
            GenUtils.initColumnField(column, table.getTableId(), table.getCreateBy());
            if (tableColumnMap.containsKey(column.getColumnName())) {
                GenTableColumnDTO prevColumn = tableColumnMap.get(column.getColumnName());
                ...
                if (StringUtils.isNotEmpty(prevColumn.getRequired()) && !GenUtils.isPk(column.getPk())
                        && (GenUtils.isInsert(column.getInsert()) || GenUtils.isEdit(column.getEdit()))
                        && GenUtils.isUsableColumn(column.getJavaField()) || (!GenUtils.isSuperColumn(column.getJavaField()))) {
        ...
        List<Long> delColumnIds =
                tableColumns.stream().filter(column -> !dbTableColumnNames.contains(column.getColumnName())).map(GenTableColumnDTO::getColumnId).toList();
        if (CollectionUtils.isNotEmpty(delColumnIds)) {
            genTableColumnMapper.deleteGenTableColumns(delColumnIds);
        }
    }
     ...
     */
    @Override
    public void validateEdit(GenTableParam genTableParam) {
        GenTable genTable = genTableParam.getGenTable();
    ...
    public void setPkColumn(GenTableDTO table) {
        for (GenTableColumnDTO column : table.getColumns()) {
            if (GenUtils.isPk(column.getPk())) {
        ...
        if (GenConstants.TPL_SUB.equals(table.getTplCategory())) {
            for (GenTableColumnDTO column : table.getSubTable().getColumns()) {
                if (GenUtils.isPk(column.getPk())) {
     ...
     */
    public void setTableFromOptions(GenTableDTO genTable) {
