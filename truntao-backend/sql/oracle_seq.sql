...
insert into sys_config values(4, '账号自助-验证码开关',           'sys.account.captchaEnabled',    'true',          'Y', 'admin', sysdate, '', null, '是否开启登录验证码功能（true开启，false关闭）');
...
-- 14、系统访问记录
-- ----------------------------
create sequence seq_sys_login_info
 increment by 1
 start with 100
 nomaxvalue
 nominvalue
...
alter table sys_login_info add constraint pk_sys_logininfor primary key (info_id);
create index idx_sys_login_info_s on sys_login_info(status);
create index idx_sys_login_inforlt on sys_login_info(login_time);

comment on table  sys_login_info                is '系统访问记录';
comment on column sys_login_info.info_id        is '访问主键seq_seq_sys_logininfor.nextval';
