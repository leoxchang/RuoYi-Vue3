package com.truntao.common.utils.poi;
...
import com.truntao.common.annotation.Excel.Type;
...
import com.truntao.common.exception.UtilException;
import com.truntao.common.utils.DateUtils;
import com.truntao.common.utils.DictUtils;
import com.truntao.common.utils.file.FileTypeUtils;
import com.truntao.common.utils.file.FileUtils;
import com.truntao.common.utils.file.ImageUtils;
import com.truntao.common.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T> {
     ...
     */
    private static final Map<String, String> sysDictMap = new HashMap<>();
    ...
    private static final String HEADER_STYLE = "header_{}_{}";

    private static final String TOTAL = "total";

    private static final String ARIAL = "Arial";

    private static final String DATA_STYLE = "data_{}_{}_{}";
    ...
    public List<T> importExcel(String sheetName, InputStream is, int titleNum) throws IOException,
        ...
        Sheet excelSheet = StringUtils.isNotEmpty(sheetName) ? wb.getSheet(sheetName) : wb.getSheetAt(0);
        if (excelSheet == null) {
        ...
        if (isXSSFWorkbook) {
            pictures = getSheetPictures07((XSSFSheet) excelSheet);
        } else {
            pictures = getSheetPictures03((HSSFSheet) excelSheet, (HSSFWorkbook) wb);
        }
        ...
        if (rows > 0) {
            ...
            // 获取表头
            Row heard = excelSheet.getRow(titleNum);
    ...
    private Map<String, CellStyle> createStyles(Workbook wb) {
        ...
        stylesMap.put(TOTAL, style);

        stylesMap.putAll(annotationHeaderStyles(wb, stylesMap));

        stylesMap.putAll(annotationDataStyles(wb));
    ...
    private Map<String, CellStyle> annotationHeaderStyles(Workbook wb, Map<String, CellStyle> styles) {
        ...
        for (Object[] os : fields) {
            Excel excel = (Excel) os[1];
            String key = CharSequenceUtil.format(HEADER_STYLE, excel.headerColor(), excel.headerBackgroundColor());
    ...
    private Map<String, CellStyle> annotationDataStyles(Workbook wb) {
        ...
        for (Object[] os : fields) {
            Excel excel = (Excel) os[1];
            String key = CharSequenceUtil.format("data_{}_{}_{}_{}_{}", excel.align(), excel.color(),
                    excel.backgroundColor(), excel.cellType(), excel.wrapText());
            stylesMap.computeIfAbsent(key, k -> {
                CellStyle style = wb.createCellStyle();
    ...
    public Cell createHeadCell(Excel attr, Row row, int column) {
        ...
        cell.setCellStyle(styles.get(CharSequenceUtil.format(HEADER_STYLE, attr.headerColor(),
                attr.headerBackgroundColor())));
        if (isSubList()) {
            // 填充默认样式，防止合并单元格样式失效
            sheet.setDefaultColumnStyle(column, styles.get(CharSequenceUtil.format("data_{}_{}_{}_{}_{}",
    ...
    public int getImageType(byte[] value) {
        String fileExtendName = FileTypeUtils.getFileExtendName(value);
        if ("JPG".equalsIgnoreCase(fileExtendName)) {
            return Workbook.PICTURE_TYPE_JPEG;
        } else if ("PNG".equalsIgnoreCase(fileExtendName)) {
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                cell.setCellStyle(styles.get(CharSequenceUtil.format("data_{}_{}_{}_{}_{}", attr.align(),
                        attr.color(), attr.backgroundColor(), attr.cellType(), attr.wrapText())));
                ...
                } else if (value instanceof BigDecimal bigDecimal && -1 != attr.scale()) {
                    cell.setCellValue((bigDecimal.setScale(attr.scale(), attr.roundingMode())).doubleValue());
                } else if (!attr.handler().equals(ExcelHandlerAdapter.class)) {
    ...
    public String dataFormatHandlerAdapter(Object value, Excel excel, Cell cell) {
        try {
            ...
            Method formatMethod = excel.handler().getMethod("format", Object.class, String[].class, Cell.class,
                    Workbook.class);
    ...
    private void addStatisticsData(Integer index, String text, Excel entity) {
        if (entity != null && entity.isStatistics()) {
            Double temp = 0D;
            statistics.putIfAbsent(index, temp);
            try {
    ...
    public String getAbsoluteFile(String filename) {
        String downloadPath = TruntaoConfig.getDownloadPath() + filename;
     ...
     * @return value
     */
    private Object getValue(Object o, String name) throws NoSuchFieldException, IllegalAccessException {
    ...
    private void createExcelField() {
        ...
        this.fields =
                this.fields.stream().sorted(Comparator.comparing(objects -> ((Excel) objects[1]).sort())).toList();
    ...
    public short getRowHeight() {
        ...
        for (Object[] os : this.fields) {
            Excel excel = (Excel) os[1];
            maxHeightVal = Math.max(maxHeightVal, excel.height());
        }
        return (short) (maxHeightVal * 20);
    }
    ...
    public String parseDateToStr(String dateFormat, Object val) {
        ...
        if (val instanceof Date date) {
            str = DateUtils.parseDateToStr(dateFormat, date);
        } else if (val instanceof LocalDateTime localDateTime) {
            str = DateUtils.parseDateToStr(dateFormat, DateUtils.toDate(localDateTime));
        } else if (val instanceof LocalDate localDate) {
            str = DateUtils.parseDateToStr(dateFormat, DateUtils.toDate(localDate));
        } else {
            str = val.toString();
        }
        return str;
    ...
    public boolean isSubListValue(T vo) {
        return Objects.nonNull(subFields) && !subFields.isEmpty() && Objects.nonNull(getListCellValue(vo)) && !getListCellValue(vo).isEmpty();
    }
