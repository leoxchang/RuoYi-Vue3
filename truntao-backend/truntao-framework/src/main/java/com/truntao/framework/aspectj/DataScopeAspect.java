...
import com.truntao.framework.security.context.PermissionContextHolder;
...
public class DataScopeAspect {
    ...
    protected void handleDataScope(final JoinPoint joinPoint, DataScope controllerDataScope) {
        ...
        if (Objects.nonNull(loginUser)) {
            SysUserDTO currentUser = loginUser.getUser();
            // 如果是超级管理员，则不过滤数据
        ...
        for (SysRoleDTO role : user.getRoles()) {
            ...
            if (DATA_SCOPE_ALL.equals(dataScope)) {
                ...
                if (StringUtils.isNotBlank(userAlias)) {
                    ...
                    sqlString.append(CharSequenceUtil.format(" OR {}.dept_id = 0 ", deptAlias));
                }
            }
            conditions.add(dataScope);
        }
