<template>
  <div class="app-container">
    ...
    <!-- 添加或修改用户配置对话框 -->
    <el-dialog :title="title" v-model="open" width="600px" :modal-append-to-body="false">
...
import {
  changeUserStatus, listUser, resetUserPwd, delUser, getUser, updateUser, addUser, deptTreeSelect
...
const deptOptions = ref<DeptOption[] | undefined>(undefined);
const enabledDeptOptions = ref<DeptOption[] | undefined>(undefined);
...
const data = reactive({
  ...
  rules: {
    ...
    password: [
      ...
      { min: 12, max: 20, message: '用户密码长度必须介于 12 和 20 之间', trigger: 'blur' },
      {
...
/** 通过条件过滤节点  */
const filterNode = (value: string, data: DeptOption): boolean => {
...
/** 过滤禁用的部门 */
function filterDisabledDept(deptList: DeptOption[]): DeptOption[] {
...
/** 节点单击事件 */
function handleNodeClick(data: DeptOption) {
...
function handleResetPwd(row: User) {
  proxy!.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
    ...
    inputPattern: /^(?![a-zA-z]+$)(?!\d+$)(?![!@#$%^&*]+$)(?![a-zA-z\d]+$)(?![a-zA-z!@#$%^&*]+$)(?![\d!@#$%^&*]+$)[a-zA-Z\d!@#$%^&*].{11,20}$/,
    inputErrorMessage: "请输入大/小写字母+数字+特殊字符,至少12位",
